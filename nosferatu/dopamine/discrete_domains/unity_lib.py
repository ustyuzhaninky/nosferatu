...
def nature_dqn_network(num_actions, network_type, state):
  ...
  net = slim.Dense(net, 512)
  q_values = slim.Dense(net, num_actions, activation_fn=None)
...
def implicit_quantile_network(num_actions, quantile_embedding_dim,
  ...
  quantiles = tf.compat.v1.random_uniform_initializer(
  ...
  quantile_net = slim.Dense(quantile_net, state_net_size,
                                      weights_initializer=weights_initializer)
  # Hadamard product.
...
class OTCPreprocessing(object):
  ...
  def step(self, action):
    ...
    if(len(observation.shape)> 2):
      observation = cv2.cvtColor(observation, cv2.COLOR_RGB2GRAY)#cv2.COLOR_RGB2RGBA)
